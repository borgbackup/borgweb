...
},{"./src/backup":4,"./src/env":5,"./src/i18n":6,"./src/util":7,"./src/viewer":8,"jquery":3}],2:[function(require,module,exports){
/*
 * Date Format 1.2.3
...
function noBackupRunning(callback) {
  $.getJSON('backup/status', function (resp) {
    ...
    if (backupRunning) util.log("\u25B6 Backup in progress");else util.log("\u2716 No backup in progress");
...
function stopBackup() {
  ...
  $.post('backup/stop', {}, function (res) {
    util.log("Message: '".concat(res.msg, "', RC: '").concat(res.rc, "'"));
  });
}
...
function startBackup(force) {
  if (force) {
    ...
    if (force === undefined) noBackupRunning(startBackup);else stopBackup();
  }
}
...
},{"./env":5,"./util":7,"./viewer":8,"jquery":3}],5:[function(require,module,exports){
"use strict";
...
var notEnglish = lang.indexOf('en') !== 0;

function translateAttribute(i18nElements, attr, translations) {
  for (var i = 0; i18nElements[i]; i++) {
...
function replaceMarkedTexts(translations) {
  ...
  var i18nElements = document.getElementsByTagName('x-i18n');
...
},{"./util":7,"jquery":3}],7:[function(require,module,exports){
"use strict";
...
function determineLineCount() {
  var availableLines = Math.floor($('#log-text').height() / 18);
...
},{"dateformat":2,"jquery":3}],8:[function(require,module,exports){
"use strict";
...
var noLogsError = $('#no-logs-error');

function highlightListEntry(id) {
  $('.shown-log').toggleClass('shown-log');
  $("#log-".concat(id)).toggleClass('shown-log');
}

function setListItemStatus() {
  var _loop = function _loop(i) {
    $.getJSON('logs/' + i, function (res) {
      var search = "#log-".concat(i, " .glyphicon");
      var elem = $(search);
      elem.css('color', env.icon[res.status][1]);
      elem.removeClass('glyphicon-time');
      elem.addClass("glyphicon-".concat(env.icon[res.status][0]));
    });
  };

  for (var i = 0; i < env.fetchRecentLogsStatus; i++) {
    _loop(i);
  }
}
...
function updateLogFileList() {
  ...
  $.getJSON('logs', function (res) {
    var i = 0;
    $.each(res.files, function (key, value) {
      var insert = i < env.fetchRecentLogsStatus;
      var indicatorHTML = insert ? "\n        <span class=\"glyphicon glyphicon-time list-status-indicator\"\n          aria-hidden=\"true\"></span>" : '';
      logFilesListHTML += "\n        <li>\n          <a onClick=\"window.switchToLog(".concat(value[0] + 1, ")\" id=\"log-").concat(value[0], "\">\n            ").concat(indicatorHTML, "\n            ").concat(value[1], "\n          </a>\n        </li>");
      i++;
    });
    setListItemStatus();
...
function getSetState(state) {
  state = state || {};
  var anchor = util.parseAnchor();
  anchor = {
    log: state.log || anchor.log || 1,
    offset: state.offset || anchor.offset || 1
  };
  document.location.hash = "#log:".concat(anchor.log, ";offset:").concat(anchor.offset);
  return anchor;
}

function updatePathAndStatus(id) {
  if (id + 1 === env.lastLogID) ;else {
    $.getJSON('logs/' + id, function (res) {
      $('#log-path').html("\n        <!-- js generated -->\n          <span class=\"glyphicon glyphicon-".concat(env.icon[res.status][0], "\"\n            aria-hidden=\"true\" style=\"font-size: 34px;\n            color: ").concat(env.icon[res.status][1], "; width: 42px;\n            margin-right: 4px; vertical-align: middle;\"></span\n          ><input class=\"form-control\" type=\"text\"\n            value=\"").concat(res.filename, "\" readonly onClick=\"this.select();\">\n        <!-- /js generated -->"));
...
function insertLogData(linesArray) {
  var html = "",
      lineStatus = "";
  linesArray.forEach(function (val, index) {
    lineStatus = env.logLine[val[0]];
    html = lineStatus ? "<mark class=\"".concat(env.logLine[val[0]][0], "\"\n      style=\"background-color: ").concat(env.logLine[val[0]][1], ";\">") : "";
    html += val[1] + '\n';
...
function displayLogSection(state, availableLines) {
  var url = "logs/".concat(state.log - 1, "/").concat(state.offset - 1, ":").concat(availableLines, ":1");
...
function getNextOffset(state, direction, availableLines, callback) {
  var url = "logs/".concat(state.log - 1, "/").concat(state.offset - 1) + ":".concat(availableLines, ":").concat(direction);
  $.get(url, function (res) {
    var subsequentUrl = "logs/".concat(state.log - 1, "/").concat(res.offset + 1) + ":".concat(availableLines, ":").concat(direction);
    $.get(subsequentUrl, function (subsequentRes) {
      if (subsequentRes.lines.length === 0) getSetState(state);else callback(state, res, availableLines);
    });
  });
}
...
function switchPage(direction) {
  ...
  getNextOffset(getSetState(), direction, availableLines, function (state, res, availableLines) {
    getSetState({
      log: state.log,
      offset: res.offset + 1
    });
...
function lastPage() {
  var state = getSetState();
  var url = "logs/".concat(state.log - 1);
  $.get(url, function (res) {
    var logLength = res.length;
