...
function startBackup(force) {
  ...
  } else if (force === undefined) noBackupRunning(startBackup);else stopBackup();
}
...
function getSetState(state) {
  ...
  document.location.hash = '#log:' + anchor.log + ';offset:' + anchor.offset;
  return anchor;
}

function updatePathAndStatus(state) {
  if (state.log === env.lastLogID) ;else $.getJSON('logs/' + (+state.log - 1), function (res) {
...
function displayLogSection(state, availableLines) {
  var url = 'logs/' + (state.log - 1) + '/' + (state.offset - 1) + ':' + availableLines + ':1';
...
function getNextOffset(state, direction, availableLines, callback) {
  var url = 'logs/' + (state.log - 1) + '/' + (state.offset - 1) + (':' + availableLines + ':' + direction);
  $.get(url, function (res) {
    var subsequentUrl = 'logs/' + (state.log - 1) + '/' + (res.offset + 1) + (':' + availableLines + ':' + direction);
