...
function pollBackupStatus(endpoint, ms, callback) {
  noBackupRunning(function (notRunning) {
    if (notRunning) {
      ...
      viewer.updateLogFileList(viewer.getCurrentRepo()); // viewer.switchToLog(1)
    } else {
...
function setListItemStatus() {
  var _loop = function _loop(i) {
    $.getJSON('logs/' + currentRepo + "/" + i, function (res) {
...
function updateLogFileList(repo) {
  ...
  $.getJSON('logs/' + repo, function (res) {
    var i = 0;
...
function updateRepoList() {
  var repoListHtml = [];
  $.getJSON('repos', function (res) {
    var i = 0;
    $.each(res, function (name, value) {
      repoListHtml += "\n        <li>\n          <a onClick=\"window.getLogFiles('".concat(name, "')\" >\n          <span class=\"glyphicon glyphicon-hdd\"\n          aria-hidden=\"true\"></span>\n            ").concat(name, "\n          </a>\n        </li>");
      i++;
    });
...
function displayLogSection(state, availableLines) {
  var url = "logs/" + currentRepo + "/" + "".concat(state.log - 1, "/").concat(state.offset - 1, ":").concat(availableLines, ":1");
...
function getNextOffset(state, direction, availableLines, callback) {
  var url = "logs/" + currentRepo + "/" + "".concat(state.log - 1, "/").concat(state.offset - 1) + ":".concat(availableLines, ":").concat(direction);
  console.log(url);
  $.get(url, function (res) {
    var subsequentUrl = "logs/" + currentRepo + "/" + "".concat(state.log - 1, "/").concat(res.offset + 1) + ":".concat(availableLines, ":").concat(direction);
...
function lastPage() {
  var state = getSetState();
  var url = "logs/" + currentRepo + "/" + +"".concat(state.log - 1);
  $.get(url, function (res) {
